        ...
        function PortfolioSimulator() {
            ...
            // Tax assumptions
            const [taxBracket, setTaxBracket] = useState(22); // Current tax bracket
            ...
            const [engagementLevel, setEngagementLevel] = useState(1); // 1-4 stages
            ...
            const volatilityChartInstance = useRef(null);
            ...
            useEffect(() => {
                ...
                if (saved) {
                    try {
                        ...
                        setStrategyMix(config.strategyMix || { growth: 40, income: 20, diversification: 40 });
                        setWithdrawalPeriods(config.withdrawalPeriods || withdrawalPeriods);
            ...
            }, [portfolio, currentIncome, savingsRate, savingsYears, strategyMix, withdrawalPeriods, inflationRate, simulations]);
            ...
            }, [portfolio, simulations, getSimulationYears, generateMarketReturns, getAnnualCashFlow, getEffectiveParameters]);
            ...
            const runSimulation = useCallback(async () => {
                ...
                try {
                    ...
                    if (strategyMix.growth !== 100 && strategyMix.income !== 100 && strategyMix.diversification !== 100) {
                        const comparisons = {};
                        comparisons.growth = runSimulationForMix({ growth: 100, income: 0, diversification: 0 });
                        comparisons.conservative = runSimulationForMix({ growth: 20, income: 40, diversification: 40 });
            ...
            const addWithdrawalPeriod = useCallback(() => {
                ...
                const newStart = lastPeriod ? lastPeriod.endYear + 1 : 1;
            ...
            const PlainLanguageLabel = ({ technical, plain, tooltip }) => (
                <label className="block text-sm font-medium text-gray-700">
                    <span className="sm:hidden">{plain}</span>
                    <span className="hidden sm:inline">{technical}</span>
                    {tooltip && <InfoTooltip text={tooltip.text} example={tooltip.example} />}
                </label>
            );
